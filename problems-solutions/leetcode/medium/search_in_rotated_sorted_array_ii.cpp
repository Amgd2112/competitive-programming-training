...
int rotatedBinarySearch(vector<int>& nums, int target, int low, int high) {
  ...
  if (nums[mid] == target) {
    ...
    int idx = rotatedBinarySearch(nums, target, mid + 1, high - 1);
    if (idx != -1) return idx;
    return rotatedBinarySearch(nums, target, low, mid - 1);
  } else {
    int idx = rotatedBinarySearch(nums, target, low + 1, mid - 1);
    if (idx != -1) return idx;
    return rotatedBinarySearch(nums, target, mid + 1, high);
  }
}
...
int search(vector<int>& nums, int target) {
  return rotatedBinarySearch(nums, target, 0, nums.size() - 1) != -1;
}
